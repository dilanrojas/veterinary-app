...
export default function PetDetails() {
  ...
  const handleRemove = () => {
    const confirmation = confirm(
      `Do you want to remove ${pet.name} from the list?`,
    );
  ...
  return (
    <div data-cy="pet-profile-page" className="bg-gray-50 dark:bg-[#0a0f0b] min-h-screen text-[#111813] dark:text-white p-4 md:p-10 font-sans">
      <div className="max-w-5xl mx-auto">
        <div className="flex items-center gap-4 mb-8">
          ...
          <h1 data-cy="pet-name" className="text-3xl font-black">
            {pet.name}{" "}
            <span data-cy="pet-type" className="text-emerald-500 text-lg capitalize">
              ({pet.type})
            </span>
          </h1>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div className="bg-white dark:bg-[#152a1c] p-6 rounded-2xl border dark:border-[#2a3a2f] text-center shadow-sm">
              ...
              <div className="text-left space-y-3 mt-6">
                <p className="flex justify-between text-sm">
                  <strong>Breed:</strong> <span data-cy="pet-breed">{pet.breed}</span>
                </p>
                <p className="flex justify-between text-sm">
                  <strong>Age:</strong> <span data-cy="pet-age">{pet.age}{" "}
                  {pet.age === 1 ? "year" : "years"}</span>
                </p>
                <p className="flex justify-between text-sm">
                  <strong>Weight:</strong> <span data-cy="pet-weight">{pet.weight} kg</span>
                </p>
                <p className="flex justify-between text-sm border-t pt-2 mt-2 font-bold text-emerald-600">
                  <span>Next Appt:</span> <span data-cy="next-appointment">{user.nextAppointment}</span>
                </p>
              </div>
            </div>
          </div>
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <div className="bg-white dark:bg-[#152a1c] p-6 rounded-2xl border dark:border-[#2a3a2f] shadow-sm min-h-[300px]">
                ...
                <div className="space-y-6">
                  <div className="bg-emerald-50 dark:bg-emerald-900/10 p-4 rounded-xl border border-emerald-100 dark:border-emerald-800">
                    <p className="font-bold text-emerald-700 dark:text-emerald-400 italic">
                      "The pet is in excellent physical condition following the
                      last check-up."
                    </p>
                  </div>
                  <div>
                    <h3 className="font-bold mb-4">Weight Chart (History)</h3>
              ...
              {tab === "Vaccines" && (
                <ul data-cy="vaccines-list" className="space-y-4">
                  <li data-cy="vaccine-item" className="flex items-center gap-4 p-3 bg-gray-50 dark:bg-[#0a0f0b] rounded-lg">
              ...
              {tab === "Visits" && (
                <ul data-cy="visits-list" className="space-y-4">
                  <li data-cy="visit-item" className="border-l-4 border-blue-500 pl-4 py-2">
                    <p className="font-bold">Weight Control</p>
                    <p className="text-sm text-gray-500">
                      January 20 - Everything under control.
                    </p>
                  </li>
                  <li data-cy="visit-item" className="border-l-4 border-emerald-500 pl-4 py-2 bg-emerald-50/30 dark:bg-emerald-900/10 rounded-r-lg">
