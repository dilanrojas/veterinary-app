...
export default function PetCard(props: Pet & { nextAppointment: string }) {
  return (
    <div
      className="flex flex-col overflow-hidden rounded-xl bg-white dark:bg-emerald-900/10 border border-[#dbe6df] dark:border-emerald-900/30 hover:shadow-xl transition-all group cursor-pointer"
      data-cy="pet-card"
    >
      <div
      ...
      <div className="p-5 flex flex-col gap-3">
        <div className="flex justify-between items-start">
          <div>
            <p
              className="text-[#111813] dark:text-white text-xl font-bold leading-tight"
              data-cy="pet-name"
            >
              {props.name}
            </p>
        ...
        <div className="flex items-center gap-2 py-2 px-3 rounded-lg bg-gray-50 dark:bg-background-dark/50">
          ...
          <p
            className="text-[#111813] dark:text-gray-300 text-xs font-semibold"
            data-cy="pet-appointment"
          >
        ...
        <Link to={`/petlist/${props.id}`}>
          <button
            className="mt-2 flex w-full items-center justify-center gap-2 rounded-lg bg-emerald-500 py-2.5 text-sm font-bold text-white hover:bg-emerald-600 transition-all"
